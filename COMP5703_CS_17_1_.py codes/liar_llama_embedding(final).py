# -*- coding: utf-8 -*-
"""LIAR_LLaMA_Embedding(Final).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1M9rgAwWzo6p7mMZbpEK1_j9Xu1Y53jTP
...
# Different numbers correspond to different data sets
dataset = "3"
...
def embedding_data(sets):
    ...
    for batch in tqdm(loader):
        try:
            ...
            batch = {k: v.to(device) for k, v in batch.items()}  # This assumes that batch is a dictionary

            out = model(**batch)

            # Processes model output and saves features to the CPU
            embedding = out.past_key_values[0][0][0].mean(axis=-1).reshape(-1).detach().cpu().numpy()

            data.append(embedding)
            label.append(batch["labels"].cpu().item())  # Moving the label back to the CPU
...
model = FakeNewsClassifierCNN().to(device)  # Make sure the model is on the GPU
...
for epoch in range(num_epochs):
    ...
    for batch_x, batch_y in train_loader:
        ...
        # Adjust the shape of batch_y to match the model output
    ...
    with torch.no_grad():
        for batch_x, batch_y in val_loader:
            batch_x, batch_y = batch_x.to(device), batch_y.to(device)
            batch_y = batch_y.view(-1, 1).float()  # Also make sure that batch_y is of floating point type and has the right shape
            outputs = model(batch_x)
