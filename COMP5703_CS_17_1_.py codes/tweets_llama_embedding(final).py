# -*- coding: utf-8 -*-
"""Tweets_LLaMA_Embedding(Final).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1FOsN0Am02Lu1lm-Y7oB4e1stkrHMdfBj
...
def embedding_data(sets):
    loader = DataLoader(sets, shuffle=False, batch_size=1)  # 您可以尝试增加批大小以进一步加快处理速度
    data, label = [], []
    for batch in tqdm(loader):
        try:
            # 将数据移动到GPU
            batch = {k: v.to(device) for k, v in batch.items()}  # 这假设batch是一个字典
            ...
            label.append(batch["labels"].cpu().item())  # 将标签移回CPU
